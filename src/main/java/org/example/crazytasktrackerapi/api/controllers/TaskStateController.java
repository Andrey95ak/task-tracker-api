...
import org.example.crazytasktrackerapi.api.dto.TaskStateDto;
...
import org.example.crazytasktrackerapi.api.factories.TaskStateDtoFactory;
import org.example.crazytasktrackerapi.store.entities.ProjectEntity;
import org.example.crazytasktrackerapi.store.entities.TaskStateEntity;
import org.example.crazytasktrackerapi.store.repositories.TaskStateRepository;
...
public class TaskStateController {
        ...
        for (TaskStateEntity taskState : project.getTaskStates()) {
            ...
            if (taskState.getName().equalsIgnoreCase(taskStateName)) {
                throw new BadRequestException(String.format(
                        "Task state with \"%s\" already exists", taskStateName
                ));
            }
        ...
        Optional<TaskStateEntity> optionalNewLeftTaskState = optionalLeftTaskStateId
                .map(leftTaskStateId -> {
                    ...
                    if (!project.getId().equals(leftTaskStateEntity.getProject().getId())) {
                        throw new BadRequestException("Task state position can be within the same project");
                    }
    ...
    public void replaceOldTaskStatePosition(TaskStateEntity changeTaskState) {
        ...
        Optional<TaskStateEntity> optionalLeftTaskState = changeTaskState.getLeftTaskState();
        Optional<TaskStateEntity> optionalRightTaskState = changeTaskState.getRightTaskState();
        ...
        return taskStateRepository
                ...
                .orElseThrow(() ->
                        new NotFoundException(String.format(
                                "Task state with \"%s\" does`t exists", taskStateId
                        )));
